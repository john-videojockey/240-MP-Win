...
FocusScope {
    ...
    ListView {
        ...
        Keys.onPressed: function(event) {
            ...
            if (event.key === Qt.Key_PageDown) { NavUtil.page(userList, 1); event.accepted = true; return }
            if (event.key === Qt.Key_PageUp) { NavUtil.page(userList, -1); event.accepted = true; return }
            if (event.key === Qt.Key_Escape || event.key === Qt.Key_Backspace || event.key === Qt.Key_Back) {
        ...
        delegate: Item {
            ...
            MouseArea {
                anchors.fill: parent
                onClicked: {
                    if (userList.currentIndex === index) inputManager.touchKey("select")
                    else userList.currentIndex = index
                }
            }
