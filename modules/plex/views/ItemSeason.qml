...
FocusScope {
    ...
    Keys.onPressed: function(event) {
        // PgUp/PgDown page the episode list a screenful at a time (only while it
        // holds the highlight), cursor kept in place.
        if (focusRow === 1 && event.key === Qt.Key_PageDown) { NavUtil.page(episodeList, 1); event.accepted = true; return }
        if (focusRow === 1 && event.key === Qt.Key_PageUp) { NavUtil.page(episodeList, -1); event.accepted = true; return }
    ...
    Image {
        ...
        anchors.fill: parent
        z: -1
        visible: seasonRoot.infoBg && status === Image.Ready
        opacity: seasonRoot.infoBgOpacity
        fillMode: Image.PreserveAspectCrop
        asynchronous: true
        source: (seasonRoot.infoBg && item.art)
        ...
        ListView {
            ...
            delegate: Item {
                ...
                MouseArea {
                    anchors.fill: parent
                    onClicked: {
                        if (focusRow === 1 && episodeList.currentIndex === index) {
