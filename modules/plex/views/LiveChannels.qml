...
FocusScope {
    ...
    ListView {
        ...
        Keys.onPressed: function(event) {
            ...
            if (event.key === Qt.Key_PageDown) { NavUtil.page(channelList, 1); event.accepted = true; return }
            if (event.key === Qt.Key_PageUp) { NavUtil.page(channelList, -1); event.accepted = true; return }
        ...
        delegate: Item {
            ...
            MouseArea {
                anchors.fill: parent
                onClicked: {
                    if (channelList.currentIndex === index) inputManager.touchKey("select")
                    else channelList.currentIndex = index
                }
            }
