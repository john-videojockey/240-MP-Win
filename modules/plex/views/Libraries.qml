...
FocusScope {
    ...
    ListView {
        ...
        Keys.onPressed: function(event) {
            ...
            if (event.key === Qt.Key_PageDown) { NavUtil.page(libraryList, 1); event.accepted = true; return }
            if (event.key === Qt.Key_PageUp) { NavUtil.page(libraryList, -1); event.accepted = true; return }
        ...
        delegate: Item {
            ...
            MouseArea {
                anchors.fill: parent
                onClicked: {
                    if (libraryList.currentIndex === index) inputManager.touchKey("select")
                    else libraryList.currentIndex = index
                }
            }
