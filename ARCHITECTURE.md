...
`mpv-osc.lua` is also fully mouse-driven (Plex-app style): the controls appear on mouse movement or click and hide after 3 s of stillness or a click outside them. It draws a title line (from `--force-media-title`, passed per-playback by the module), a close X, a click-to-seek bar, and `|<` / `PLAY|PAUSE` / `>|` / `AUDIO` / `SUBTITLE` / `CROP` / `STOP` buttons. One `layout()` function computes every rectangle for both drawing and click hit-testing so they can't drift apart. `>|` runs `playlist-next` when an mpv playlist is loaded; otherwise, when the app opted in with `--script-opts-append=episode-nav=1`, it sends the `episode-nav` client-message, which `MpvController` re-emits as `episodeNavRequested` — the Plex Player answers it by swapping to the next episode in place (same machinery as autoplay). `|<` restarts the current item. The keyboard/remote flow (UP/DOWN opens, arrows + ENTER navigate, 5 s timeout) is unchanged.
